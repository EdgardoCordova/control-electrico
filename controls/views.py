...
from controls.models import crc_ONOFF

def controls_list_view(request):
    qs1 = crc_PROG.objects.all()
    qs2 = crc_ONOFF.objects.all()
    ...
    return render(request, 'controls/main.html', context)


def batch_crc_ONOFF_view(request):
    sum_events = 0
    i = 1
    qs1 = crc_PROG.objects.all()
    qs2 = crc_ONOFF.objects.all()
    if qs2:
        crc_ONOFF.objects.all().delete()
    a=0
    i=0
    sum_events = 0
